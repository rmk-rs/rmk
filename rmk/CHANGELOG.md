...
- Add configurable Sticky Keys. `SK(action)` defers the release of any single action — a modifier, a layer, a plain keycode, `WM`, a macro — until a release trigger ends it, with named profiles for the timeout, the repeat cap, activation reporting, a hold threshold, the release triggers, and a `keep_keys`/`release_keys` list saying which keys end the latch. `OSM`/`OSL` remain compatibility aliases, and legacy `[behavior.one_shot_modifiers] quick_release` still selects press-triggered release for the default profile
