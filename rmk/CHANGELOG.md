...
- Identify the keyboard's HID report characteristics on the dongle by their Report Reference descriptor instead of `HidService`'s declaration order, so a keyboard firmware that adds or reorders reports keeps working with an older dongle
- Fix the ESP32 display I2C initialization generated by `#[rmk_keyboard]`: it called `with_sda` on the `Result` returned by esp-hal's `I2c::new` and handed the display driver a blocking bus, so any ESP32 `keyboard.toml` with a `[display]` section failed to compile
...
- Suppress split peripheral BLE battery notifications while the keyboard is sleeping, and resume reporting after wake or recent activity
