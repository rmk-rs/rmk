//! Wire-format request payloads for the BLE DFU protocol.
...
/// and the data length varint (2 bytes for sizes up to 16 383).  The host
...
pub const DFU_DATA_MAX: usize = {
    ...
    // Overhead: offset u32 varint (5) + data length varint (2 for ≤16383)
    let overhead = 7;
