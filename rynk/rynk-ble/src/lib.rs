...
impl BleDevice {
    ...
    /// which advertise with a name like `"rmk per0"` (the default; the
    /// firmware's `dfu_name` argument can override it).
    ...
    async fn discover_characteristic(&self) -> Result<(Characteristic, Characteristic), RynkHostError> {
        ...
        match self
            ...
            .await
        {
            Ok(Some(chars)) => return Ok(chars),
            Ok(None) => {}
            Err(e) => log::debug!("rynk-ble: Rynk service discovery failed ({e:?}), trying DFU service"),
        }
