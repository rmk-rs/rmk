...
use rmk_types::protocol::rynk::{
    ...
    GetMacroRequest, GetMorseBulkRequest, GetMorseBulkResponse, KeyPosition, LockStatus, MacroData, MatrixState,
    PeripheralStatus, ProtocolVersion, SetComboBulkRequest, SetComboRequest, SetEncoderRequest, SetForkRequest,
    SetKeyRequest, SetKeymapBulkRequest, SetMacroRequest, SetMorseBulkRequest, SetMorseRequest, StorageResetMode,
    command,
};
#[cfg(feature = "alloc")]
use rmk_types::protocol::rynk::{DfuWriteRequest, RYNK_HEADER_SIZE, RynkError, max_wire_size};
#[cfg(feature = "alloc")]
use serde::Serialize;
...
impl Client {
    ...
    /// Reject DFU commands when the device doesn't advertise `dfu_enabled`.
    ...
    ///
    /// Fire-and-forget: the frame is queued for the writer without waiting
    /// for the firmware's reply (which arrives unmatched and is dropped).
    /// Use periodic [`dfu_crc_sync`](Self::dfu_crc_sync) to verify receipt.
    #[cfg(feature = "alloc")]
    pub async fn dfu_write(&self, offset: u32, data: Vec<u8>) -> Result<(), RynkHostError> {
        self.require_dfu(Cmd::DfuWrite)?;
        self.send_no_reply::<command::DfuWrite>(&DfuWriteRequest { offset, data })
            .await
    }
