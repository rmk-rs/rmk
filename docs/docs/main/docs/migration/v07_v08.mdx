...
- **Simplified matrix configuration**: Replaced `input/output_pins` with `row/col_pins` in `keyboard.toml` and made the `col2row` Cargo feature a no-op — diode direction is now set by `row2col` in `[matrix]` (the feature declaration itself is removed in v0.9). This makes matrix setup more intuitive.
