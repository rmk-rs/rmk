...
project. The project's `build.rs` (from the project template or an example) compresses it into
`VIAL_KEYBOARD_DEF` in `OUT_DIR/config_generated.rs`, which the `#[rmk_keyboard]` macro includes. If you
