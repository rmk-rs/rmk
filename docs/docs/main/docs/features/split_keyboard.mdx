...
RMK supports multi-split keyboard, which contains one central board and up to [`split_peripherals_num`](../configuration/rmk_config) peripheral boards. The host is connected to the central board via USB or BLE.

::: info

There are many examples for split keyboards in [RMK examples folder](https://github.com/rmk-rs/rmk/tree/main/examples/use_config).
...
BLE split needs the [storage](./storage.md) feature; every BLE chip feature enables it automatically.
