...
| `0x0901` | `DfuStart`            | `()`                   | `()`                    |         | Start a DFU download session. Requires unlock when `dfu_lock` is active.                                                                            |
| `0x0902` | `DfuWrite`            | `DfuWriteRequest`      | `()`                    |         | Write a chunk of firmware data at the given offset.                                                                                                 |
| `0x0903` | `DfuFinish`           | `()`                   | `()`                    |         | Finalize the DFU transfer: sanity-check the image and reset into it.                                                                                |
| `0x0904` | `DfuReset`            | `()`                   | `()`                    |         | Request a hard system reset without finishing a DFU transfer.                                                                                       |
| `0x0905` | `DfuCrcSync`          | `DfuCrcSyncRequest`    | `()`                    |         | Periodic CRC-32 synchronization: host sends its running CRC, firmware compares and returns `Ok` on match or `Err(RynkError::Internal)` on mismatch. |
| `0x0906` | `DfuCrcRewind`        | `DfuCrcRewindRequest`  | `()`                    |         | Rewind the firmware's DFU state to a previous CRC checkpoint so the host can retransmit from that offset.                                           |
| `0x0907` | `DfuVerify`           | `DfuVerifyRequest`     | `()`                    |         | End-of-transfer verification: firmware reads back the DFU partition, recomputes CRC-32, and runs the vector-table sanity check.                     |
