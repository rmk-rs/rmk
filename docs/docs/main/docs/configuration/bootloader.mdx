...
When using `FlashDfuHandler` (in `run_all!`) or `run_rmk_split_peripheral()`, `mark_booted()` is called automatically — no manual call needed. You only need `rmk::dfu::mark_booted(&mut state_partition)` when using an embassy-boot based bootloader to flash firmware that still provides safe firmware updates and rollback — e.g. rmk-boot's DFU flash feature without its `noswap` feature while you don't use DFU flashing / `FlashDfuHandler` in RMK itself.
:::
...
The `[dfu]` section is **optional** and configures only DFU behaviour (LED, unlock keys). Partition offsets are read at runtime from `memory.x` — you do **not** set `state_offset`, `dfu_offset`, or `flash_size` in `keyboard.toml`.
