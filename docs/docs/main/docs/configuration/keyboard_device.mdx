...
[keyboard]
name = "My Custom Keyboard"       # Keyboard name. A label for your config; the generated firmware doesn't use it
...
hardcoded serial override. Non-Vial builds use `rmk:<version>` without the Vial marker; builds with
the `rynk` feature (or `dongle` without `vial`) prepend `rynk:` to the USB serial number, giving
`rynk:rmk:<version>`. The Vial marker comes first because BLE's serial number characteristic is
length limited, so a trailing marker can be truncated away.
