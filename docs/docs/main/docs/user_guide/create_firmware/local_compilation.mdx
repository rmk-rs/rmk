...
To compile firmware for nRF chips, you also need the [Arm GNU toolchain](https://developer.arm.com/downloads/-/arm-gnu-toolchain-downloads), which builds the P-256 assembly that RMK uses to speed up BLE pairing:
...
`memory.x` is the linker script of the Rust embedded project, it's used to define the memory layout of the microcontroller. The generated project (like the STM32 examples) enables the `memory-x` feature of `embassy-stm32`, so if you're using stm32, you can just ignore this step.
