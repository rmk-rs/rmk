...
5. Now you get your RMK firmware! The workflow uploads `uf2` and `hex` firmware (a `bin` image for ESP32 chips instead, ready for `espflash`), plus the raw `elf` for flashing with a debug probe.
